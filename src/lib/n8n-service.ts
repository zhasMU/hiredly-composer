...
class N8nService {
  ...
  private setupProgressListener() {
    ...
    if ('WebSocket' in window) {
      try {
        const ws = new WebSocket(`${this.config.baseUrl.replace('http', 'ws')}/progress`);
  ...
  onProgress(executionId: string, callback: (update: ProgressUpdate) => void) {
  ...
  async processKeywords(request: KeywordsRequest): Promise<WorkflowResponse<{ 
  ...
  async conductResearch(request: ResearchRequest): Promise<WorkflowResponse<{
    sources: Source[];
    totalFound: number;
    searchQueries: string[];
  }>> {
    return this.makeRequest('/webhook/research', request, { timeout: 60000 }); // 60 second timeout for research
  }

  // Step 3: Source Analysis & Scoring
  async analyzeSources(sources: Source[]): Promise<WorkflowResponse<{
  ...
  async generateDraft(request: DraftRequest): Promise<WorkflowResponse<ArticleContent>> {
    return this.makeRequest('/webhook/generate-draft', request, { timeout: 120000 }); // 2 minute timeout
  }

  // Step 5: Quality Analysis & Scoring
  async analyzeQuality(content: ArticleContent): Promise<WorkflowResponse<{
    metrics: QualityMetrics;
    suggestions: Array<{
    ...
    return this.makeRequest('/webhook/quality-analysis', { content });
  }

  // Step 6: Export & Formatting
  async exportArticle(content: ArticleContent, format: 'html' | 'docx' | 'pdf' | 'markdown'): Promise<WorkflowResponse<{
    downloadUrl: string;
  ...
  async cancelExecution(executionId: string): Promise<WorkflowResponse<{ cancelled: boolean }>> {
