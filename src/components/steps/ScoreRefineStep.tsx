...
const mockSuggestions: Suggestion[] = [
    {
        ...
        description:
            'Add transitional phrases between paragraphs 3 and 4 to enhance flow',
        applied: false,
    },
    {
        id: '2',
        ...
        description:
            'Ensure all citations follow consistent academic format (APA/MLA)',
        applied: false,
    },
    {
        id: '3',
        ...
        description:
            'Break down longer sections with descriptive subheadings for better readability',
        applied: true,
    },
    {
        id: '4',
        ...
        description:
            'Add specific recommendations or future research directions in the conclusion',
        applied: false,
    },
];
...
export const ScoreRefineStep = ({ workflowManager }: ScoreRefineStepProps) => {
    ...
    const overallScore = Math.round(
        mockMetrics.reduce((acc, metric) => acc + metric.score, 0) /
            mockMetrics.length
    );
    const improvementCount = suggestions.filter(
        (s) => !s.applied && s.type === 'improvement'
    ).length;
    const warningCount = suggestions.filter(
        (s) => !s.applied && s.type === 'warning'
    ).length;
    ...
    return (
        <div className="max-w-6xl mx-auto p-8 space-y-8">
            <div className="text-center space-y-4">
                ...
                <p className="text-muted-foreground">
                    Review your article's quality metrics and apply suggested
                    improvements
                </p>
            </div>
                        ...
                        {suggestions.map((suggestion) => (
                            <Card
                                ...
                                className={`transition-all ${
                                    suggestion.applied ? 'bg-muted/30' : ''
                                }`}
                            >
                                <CardContent className="p-4">
